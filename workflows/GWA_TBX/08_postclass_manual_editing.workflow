...
This post-processing workflow allow users to apply a segmentation routine to any thematic map to allow for manual editing i.e. manual selection and reclassification of segements in problematic regions.
...
This first step of the workflow assures the Nodata value is correctly defined. Evaluate the classified imaget to varify that the Nodata values should be -32768. If this is not the case change the settings here.
...
This step performs a segmentation of the mangrove classification in combination with the spectral 10m bands stacked in step 3 of this workflow in the tif file.
...
## Notes
The output file from this step is the file that can be used in the manual editing proces of the classification.
...
**Input layer**: Select the shapefile generated from step 5.

**Load fields from layer**: Select the shapefile generated from step 5 and press the "Load fields" bottom.

Now in the Field Mapping window manually mark the Rows that should be removed as descreibed below:

Mark Row "a_cat" and press the "Delete Selected field" bottom (Red x)

Mark Row "b_cat" and press the "Delete Selected field" bottom (Red x)

Mark Row "b_DN" and press the "Delete Selected field" bottom (Red x)

The field to keep is the one having informations about the class. When the only field listed in the Field mapping window is "a_Class" you are ready for creating the final output shapefile.

**Refactored**: Select the folder and the file name of the resulting shapefile that will be ready for manuel editing: 06_PostClass_['enter name'].shp

When ready press the "Run" bottom
!INSTRUCTIONS
